...
class Model(nn.Module):
    ...
    def __init__(self, hyperparameters):
        ...
        self.dataset = dataloader( self.DATASET, copy.deepcopy(self.auxiliary_data_source) , device= self.device, num_shots = self.num_shots,
                                   few_train =self.few_train, image_embedding = hyperparameters['image_embedding'], class_embedding=hyperparameters['class_embedding'])
        if self.few_train:
            self.num_shots = 0
    ...
    def generate_syn_feature(self):
        unseen_classes = self.dataset.test_classes
        attribute = self.dataset.aux_data
        num = 500
        print(unseen_classes)
        nclass = unseen_classes.shape[0]
        syn_feature = torch.FloatTensor(nclass * num, 2048)
        # syn_feature = torch.FloatTensor(nclass * num, 64)
        syn_label = torch.LongTensor(nclass * num)
        syn_att = torch.FloatTensor(num, 85)
        syn_att = syn_att.cuda()

        for i in range(nclass):
            iclass = unseen_classes[i]
            iclass_att = attribute[iclass]
            syn_att.copy_(iclass_att.repeat(num, 1))

            mu_att, logvar_att = self.encoder[self.auxiliary_data_source](syn_att)
            z_from_att = self.reparameterize(mu_att, logvar_att)
            img_from_att = self.decoder['resnet_features'](z_from_att)

            syn_feature.narrow(0, i * num, num).copy_(img_from_att.data.cpu())
            # syn_feature.narrow(0, i * num, num).copy_(z_from_att.data.cpu())
    ...
    def train_classifier(self, show_plots=False):
        ...
        with torch.no_grad():
            ...
            print(z_unseen_att.cpu().detach().numpy().shape, att_unseen_label.cpu().detach().numpy().shape, z_seen_img.cpu().detach().numpy().shape, img_seen_label.cpu().detach().numpy().shape,
                  test_novel_X.cpu().detach().numpy().shape, test_novel_Y.cpu().detach().numpy().shape, test_seen_X.cpu().detach().numpy().shape, test_seen_label.cpu().detach().numpy().shape)
            mydata = {"train_unseen_data":z_unseen_att.cpu().detach().numpy(), "train_unseen_label":att_unseen_label.cpu().detach().numpy(),
                      "train_seen_data":z_seen_img.cpu().detach().numpy(), "train_seen_label":img_seen_label.cpu().detach().numpy(),
                      "test_unseen_data":test_novel_X.cpu().detach().numpy(), "test_unseen_label":test_novel_Y.cpu().detach().numpy(),
                      "test_seen_data":test_seen_X.cpu().detach().numpy(), "test_seen_label":test_seen_label.cpu().detach().numpy()}
            savemat("cadavae_data.mat",mydata)
            print("cadavae_data.mat is saved!")
        ...
        for k in range(self.cls_train_epochs):
            ...
            if self.generalized:
                ...
                print('[%.1f]     novel=%.4f, seen=%.4f, h=%.4f , loss=%.4f, time=%.4f' % (
                k, cls.acc_novel, cls.acc_seen, cls.H, cls.average_loss, self.run_time1))
                ...
                print('[%.1f]  acc=%.4f, time=%.4f ' % (k, cls.acc, self.run_time2))
