...
class DATA_LOADER(object):
    def __init__(self, dataset, aux_datasource, device='cuda', num_shots=0, few_train=False, image_embedding = 'res101', class_embedding = 'att'):

        print("The current working directory is")
        print(os.getcwd())
        folder = str(Path(os.getcwd()).parent)
        if folder[-5:] == 'model':
    ...
    def read_matdataset(self):
        matcontent1 = sio.loadmat(self.datadir + self.image_embedding + '.mat')
        matcontent = sio.loadmat(self.datadir + self.class_embedding + '_splits.mat')
        if self.num_shots > 0 and self.few_train:
            matcontent1, matcontent = self.process_few_shot_train(matcontent1, matcontent, self.num_shots)
        ...
        self.test_classes = np.unique(test_unseen_label.cpu().numpy())
