...
# naive feature
# CUDA_VISIBLE_DEVICES=1 nohup python -u main.py --dataset AWA2 --few_train False --num_shots 0 --generalized True --image_embedding res101_naive > awa2.log 2>&1 &
...
class myArgs():
    def __init__(self, args):
        ...
        self.start_epoch = 0; self.manualSeed = 0; self.nclass_all = 200; self.Neighbours = 20; self.CENT_LAMBDA = 1;
        self.epsilon = 0.15; self.upper_epsilon = 0.04; self.correlation_penalty = 0.25; self.unseen_start = 0;
        self.no_classifier = False; self.unseen_cls_weight = 0.1;

        if self.generalized:
            if self.dataset == 'CUB':
                self.manualSeed = 3843; self.val_every = 1; self.cls_weight = 0.01; self.preprocessing = True
                self.cuda = True; self.netG_name = "MLP_G"
                self.netD_name = "MLP_CRITIC"; self.nepoch = 200; self.ngh = 4096; self.ndh = 4096; self.lr = 0.0001; self.classifier_lr = 0.001
                self.lambda1 = 10; self.critic_iter = 5; self.dataset = "CUB"; self.nclass_all = 200; self.batch_size = 64; self.nz = 312
                self.attSize = 312; self.resSize = 2048; self.syn_num = 300; self.outname = "cub"; self.no_classifier = True
                self.epsilon = 0.1; self.upper_epsilon = 0.1; self.nclass_all = 200; self.correlation_penalty = 20; self.unseen_cls_weight = 0.03
            elif self.dataset == 'FLO':
                self.manualSeed = 3843; self.val_every = 1; self.cls_weight = 0.01; self.preprocessing = True
                self.cuda = True; self.netG_name = "MLP_G"; self.netD_name = "MLP_CRITIC"
                self.nepoch = 200; self.ngh = 4096; self.ndh = 4096; self.lr = 0.0001; self.classifier_lr = 0.001; self.lambda1 = 10
                self.critic_iter = 5; self.dataset = "FLO"; self.nclass_all = 200; self.batch_size = 64; self.nz = 1024
                self.attSize = 1024; self.resSize = 2048; self.syn_num = 1600; self.outname = "cub"; self.no_classifier = True
                self.epsilon = 0.1; self.upper_epsilon = 0.1; self.nclass_all = 200; self.correlation_penalty = 20; self.unseen_cls_weight = 0.03
            elif self.dataset == 'SUN':
                self.manualSeed = 4115; self.val_every = 1; self.cls_weight = 0.03; self.preprocessing = True
                self.cuda = True; self.netG_name = "MLP_G"; self.netD_name = "MLP_CRITIC"
                self.nepoch = 200; self.ngh = 4096; self.ndh = 4096; self.lambda1 = 10; self.critic_iter = 5; self.dataset = "SUN";
                self.batch_size = 64; self.nz = 102; self.attSize = 102; self.resSize = 2048;  self.lr = 0.0002; self.syn_num = 400;
                self.classifier_lr = 0.001; self.nclass_all = 717; self.outname = "sun"; self.upper_epsilon = 0.1; self.epsilon = 0.1;
                self.correlation_penalty = 10; self.no_classifier = True; self.unseen_cls_weight = 0.01
            elif self.dataset == 'AWA1':
                self.manualSeed = 9182; self.val_every = 1; self.cls_weight = 0.001; self.preprocessing = True; self.lr = 0.00001; self.cuda = True
                self.netG_name = "MLP_G"; self.netD_name = "MLP_CRITIC"
                self.nepoch = 200;  self.syn_num = 1800; self.ngh = 4096; self.ndh = 4096; self.lambda1 = 10; self.critic_iter = 5; self.nclass_all = 50;
                self.batch_size = 64; self.nz = 85; self.attSize = 85; self.resSize = 2048; self.outname = "awa"
                self.no_classifier = True; self.epsilon = 0.15; self.upper_epsilon = 0.04; self.correlation_penalty = 20
                self.unseen_cls_weight = 0.1
            elif self.dataset == 'aPY':
                self.manualSeed = 3483; self.val_every = 1; self.cls_weight = 0.01; self.preprocessing = True;  self.cuda = True
                self.netG_name = "MLP_G"; self.netD_name = "MLP_CRITIC"
                self.nepoch = 70; self.ngh = 4096; self.ndh = 4096; self.lr = 0.0001; self.classifier_lr = 0.001; self.lambda1 = 10;
                self.critic_iter = 5; self.dataset = "aPY"; self.nclass_all = 32; self.batch_size = 64; self.nz = 64; self.attSize = 64;
                self.resSize = 2048; self.syn_num = 1600; self.outname = "apy"; self.no_classifier = True; self.epsilon = 0.1;
                self.upper_epsilon = 0.1; self.correlation_penalty = 20; self.unseen_cls_weight = 0.03;
            elif self.dataset == 'AWA2':
                self.manualSeed = 9182; self.val_every = 1; self.cls_weight = 0.001; self.preprocessing = True;  self.cuda = True
                self.netG_name = "MLP_G"; self.netD_name = "MLP_CRITIC"
                self.nepoch = 200; self.ngh = 4096; self.ndh = 4096; self.lr = 0.00001; self.lambda1 = 10;
                self.critic_iter = 5; self.nclass_all = 50; self.batch_size = 64; self.nz = 85; self.attSize = 85;
                self.resSize = 2048; self.syn_num = 1800; self.outname = "awa2"; self.no_classifier = True; self.epsilon = 0.15;
                self.upper_epsilon = 0.04; self.correlation_penalty = 20;  self.unseen_cls_weight = 0.1;
        else:
            if self.dataset == 'CUB':
                self.manualSeed = 3843; self.val_every = 1; self.cls_weight = 0.01; self.preprocessing = True
                self.cuda = True; self.netG_name = "MLP_G"
                self.netD_name = "MLP_CRITIC"; self.nepoch = 70; self.ngh = 4096; self.ndh = 4096; self.lr = 0.0001; self.classifier_lr = 0.001
                self.lambda1 = 10; self.critic_iter = 5; self.dataset = "CUB"; self.nclass_all = 200; self.batch_size = 64; self.nz = 312
                self.attSize = 312; self.resSize = 2048; self.syn_num = 300; self.outname = "cub"; self.no_classifier = True
                self.epsilon = 0.15; self.upper_epsilon = 0.15; self.correlation_penalty = 0.15; self.unseen_cls_weight = 0.01
            elif self.dataset == 'FLO':
                self.manualSeed = 3483; self.val_every = 1; self.cls_weight = 0.01; self.preprocessing = True
                self.cuda = True; self.netG_name = "MLP_G"
                self.netD_name = "MLP_CRITIC"; self.nepoch = 200; self.ngh = 4096; self.ndh = 4096; self.lr = 0.0001; self.classifier_lr = 0.001
                self.lambda1 = 10; self.critic_iter = 5; self.dataset = "FLO"; self.nclass_all = 102; self.batch_size = 64; self.nz = 1024
                self.attSize = 1024; self.resSize = 2048; self.syn_num = 300; self.outname = "flo"; self.no_classifier = True
                self.epsilon = 0.15; self.upper_epsilon = 0.15; self.correlation_penalty = 0.15; self.unseen_cls_weight = 0.01
            elif self.dataset == 'SUN':
                self.manualSeed = 4115; self.val_every = 1; self.cls_weight = 0.03; self.preprocessing = True
                self.cuda = True; self.netG_name = "MLP_G"
                self.netD_name = "MLP_CRITIC"; self.nepoch = 20; self.ngh = 4096; self.ndh = 4096; self.lambda1 = 10; self.critic_iter = 5
                self.dataset = "SUN"; self.batch_size = 64; self.nz = 102; self.attSize = 102; self.resSize = 2048; self.lr = 0.0002;
                self.classifier_lr = 0.001; self.syn_num = 400; self.outname = "sun"; self.epsilon = 0.1; self.upper_epsilon = 0.1;
                self.correlation_penalty = 10;  self.no_classifier = True; self.unseen_cls_weight = 0.01; self.nclass_all = 717
            elif self.dataset == 'AWA1':
                self.manualSeed = 9182; self.val_every = 1; self.cls_weight = 0.01; self.preprocessing = True
                self.cuda = True; self.netG_name = "MLP_G"
                self.netD_name = "MLP_CRITIC"; self.nepoch = 70; self.syn_num = 300; self.ngh = 4096; self.ndh = 4096;
                self.lambda1 = 10; self.critic_iter = 5; self.dataset = "AWA1"; self.batch_size = 64; self.nz = 85;
                self.attSize = 85; self.resSize = 2048; self.outname = "awa1"; self.nclass_all = 50; self.unseen_start = 0;
                self.epsilon = 0.15; self.upper_epsilon = 0.04; self.correlation_penalty = 20;  self.no_classifier = True;
                self.unseen_cls_weight = 0.01;  self.lr = 0.00001
            elif self.dataset == 'aPY':
                self.manualSeed = 3483; self.val_every = 1; self.cls_weight = 0.01; self.preprocessing = True
                self.cuda = True; self.netG_name = "MLP_G"
                self.netD_name = "MLP_CRITIC"; self.nepoch = 70; self.syn_num = 300; self.ngh = 4096; self.ndh = 4096;
                self.lambda1 = 10; self.critic_iter = 5; self.dataset = "aPY"; self.batch_size = 64; self.nz = 64;
                self.attSize = 64; self.resSize = 2048; self.outname = "apy"; self.nclass_all = 32;  self.epsilon = 0.15;
                self.upper_epsilon = 0.15; self.correlation_penalty = 0.15; self.no_classifier = True; self.unseen_cls_weight = 0.01;
                self.lr = 0.0001
            elif self.dataset == 'AWA2':
                self.manualSeed = 9182; self.val_every = 1; self.cls_weight = 0.01; self.preprocessing = True
                self.cuda = True; self.netG_name = "MLP_G"
                self.netD_name = "MLP_CRITIC"; self.nepoch = 70; self.syn_num = 300; self.ngh = 4096; self.ndh = 4096;
                self.lambda1 = 10; self.critic_iter = 5; self.dataset = "AWA2"; self.batch_size = 64; self.nz = 85;
                self.attSize = 85; self.resSize = 2048; self.outname = "awa2"; self.nclass_all = 50; self.unseen_start = 0;
                self.epsilon = 0.15; self.upper_epsilon = 0.04; self.correlation_penalty = 20;  self.no_classifier = True;
                self.unseen_cls_weight = 0.01;  self.lr = 0.00001


opt = myArgs(args)
print(opt.image_embedding, opt.syn_num, opt.cls_weight, opt.lr, opt.classifier_lr, opt.lambda1)
run(opt)
