...
class CLASSIFIER:
    ...
    def __init__(self, _train_X, _train_Y, data_loader, _nclass, _cuda, _lr=0.001, _beta1=0.5, _nepoch=20,
                 _batch_size=100, netFR=None, dec_size=4096, dec_hidden_size=4096, generalized=True):
        ...
        if self.netFR:
            ...
            if self.train_X.shape[0] > 46000:
                syn_feature = self.train_X[:4600*10]
                syn_label = self.train_Y[:4600*10]
                train_feature = self.train_X[4600*10:]
                train_label = self.train_Y[4600*10:]
                from scipy.io import savemat
                print(syn_feature.cpu().detach().numpy().shape, syn_label.cpu().detach().numpy().shape,
                      train_feature.cpu().detach().numpy().shape,
                      train_label.cpu().detach().numpy().shape,
                      self.test_unseen_feature.cpu().detach().numpy().shape,
                      self.test_unseen_label.cpu().detach().numpy().shape,
                      self.test_seen_feature.cpu().detach().numpy().shape,
                      self.test_seen_label.cpu().detach().numpy().shape)
                mydata = {"train_unseen_data": syn_feature.cpu().detach().numpy(),
                          "train_unseen_label": syn_label.cpu().detach().numpy(),
                          "train_seen_data": train_feature.cpu().detach().numpy(),
                          "train_seen_label": train_label.cpu().detach().numpy(),
                          "test_unseen_data": self.test_unseen_feature.cpu().detach().numpy(),
                          "test_unseen_label": self.test_unseen_label.cpu().detach().numpy(),
                          "test_seen_data": self.test_seen_feature.cpu().detach().numpy(),
                          "test_seen_label": self.test_seen_label.cpu().detach().numpy()}
                savemat("free_data.mat", mydata)
                print("free_data.mat is saved!")
