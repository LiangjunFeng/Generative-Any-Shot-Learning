...
# CUDA_VISIBLE_DEVICES=0 nohup python -u finetune.py FLO > flo_data.log 2>&1 &
# CUDA_VISIBLE_DEVICES=1 nohup python -u finetune.py AWA2 > awa2_data.log 2>&1 &
# CUDA_VISIBLE_DEVICES=2 nohup python -u finetune.py CUB > cub_data.log 2>&1 &
# CUDA_VISIBLE_DEVICES=3 nohup python -u finetune.py SUN > sun_data.log 2>&1 &
# CUDA_VISIBLE_DEVICES=0 nohup python -u finetune.py aPY > apy_data.log 2>&1 &
...
if benchmark in ["AWA2", "FLO", "SUN"]:
    data['image_files'] = np.array(list(map(lambda x: path+benchmark+'/'+'/'.join(x[0][0].split("/")[7:]), data['image_files'])))
elif benchmark == "CUB":
    data['image_files'] = np.array(list(map(lambda x: path+benchmark+'/'+'/'.join(x[0][0].split("/")[6:]), data['image_files'])))
elif benchmark == "aPY":
...
data['train_seen_index'] = list(set(np.arange(len(data['image_files'])).tolist()) - set(data['test_unseen_index']) - set(data['test_seen_index']))
...
class dataset(Dataset):
    def __init__(self,data_files, label, transform):
        super().__init__()
...
train_dataset = dataset(traindata_files,trainlabel,data_transforms['train'])
train_loader = DataLoader(train_dataset,batch_size=64,shuffle=True,num_workers=4)

all_dataset = dataset(alldata_files,alllabel,data_transforms['val'])
...
def train_model(model, criterion, optimizer, scheduler, num_epochs):
    for epoch in range(num_epochs):
        if epoch in [0,5,10,15,20,25,30,40,50,60,70,80,90,100]:
            ...
            savemat(savepath+benchmark+"/res101_finetune_SGD_lr3_" + str(epoch) + "epo.mat", {'features':data['features'].T,
            ...
            print(path+benchmark+"/res101_finetune_" + str(epoch) + "epo.mat" + " is saved!")
        ...
        print('{} Loss: {:.4f} Acc: {:.4f}'.format(epoch+1, epoch_loss, epoch_acc))
        print()
    return model
